...
angular.module('xoApp')
  .controller('GameCtrl', function ($scope, localStorageService, $timeout) {
	...
	$scope.isWinning = function() {
		if ($scope.isHorizontalThreeOccupiedByMe($scope.currentPlayer) || $scope.isVerticalThreeOccupiedByMe($scope.currentPlayer)
	 	|| $scope.isDiagonalThreeOccupiedByMe($scope.currentPlayer)) {
			return true;
		};
	}
